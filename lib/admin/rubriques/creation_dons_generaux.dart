...
class CreationDonsGeneraux extends StatefulWidget {
  const CreationDonsGeneraux({super.key});

  @override
  State<CreationDonsGeneraux> createState() => _CreationDonsGenerauxState();
}

class _CreationDonsGenerauxState extends State<CreationDonsGeneraux> {
  ...
  Future<void> _addDonationDialog() async {
    ...
    Future<void> uploadDonation() async {
      ...
      try {
        ...
        for (var image in selectedImages) {
          final storageRef = FirebaseStorage.instance.ref().child("donsGeneraux/${DateTime.now().toIso8601String()}-${image.path.split('/').last}");
          await storageRef.putFile(image);
          String imageUrl = await storageRef.getDownloadURL();
          imageUrls.add(imageUrl);
        }

        await FirebaseFirestore.instance.collection("donsGeneraux").add({
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Don ajouté avec succès !")));
      } catch (e) {
        if (!mounted) return;
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          title: const Text("Ajouter un Don Général"),
          content: Column(
  ...
  Future<void> _updateDonationDialog(DocumentSnapshot doc) async {
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          title: const Text("Modifier le Don"),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              TextField(
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                try {
                  await FirebaseFirestore.instance.collection("donsGeneraux").doc(doc.id).update({
                    "title": titleController.text,
                    "message": messageController.text,
                  });
                  ...
                  if (!mounted) return;
                  navigator.pop();
                  messenger.showSnackBar(const SnackBar(content: Text("Don mis à jour avec succès !")));
                } catch (e) {
                  if (!mounted) return;
  ...
  Future<void> _deleteDonation(DocumentSnapshot doc) async {
    try {
      ...
      await FirebaseFirestore.instance.collection("donsGeneraux").doc(doc.id).delete();
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Don supprimé avec succès")));
    } catch (e) {
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("Dons Généraux")),
      body: StreamBuilder(
        stream: FirebaseFirestore.instance.collection("donsGeneraux").orderBy("createdAt", descending: true).snapshots(),
        builder: (context, AsyncSnapshot<QuerySnapshot> snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            return const Center(child: CircularProgressIndicator());
          }
          if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
            return const Center(child: Text("Aucun don général disponible."));
          }
