...
class CreationParrainageOrphelin extends StatefulWidget {
  const CreationParrainageOrphelin({super.key});

  @override
  State<CreationParrainageOrphelin> createState() => _CreationParrainageOrphelinState();
}

class _CreationParrainageOrphelinState extends State<CreationParrainageOrphelin> {
  ...
  Future<void> _addDonationDialog() async {
    ...
    Future<void> uploadDonation() async {
      ...
      try {
        ...
        for (var image in selectedImages) {
          final storageRef = FirebaseStorage.instance.ref().child("parrainageOrphelin/${DateTime.now().toIso8601String()}-${image.path.split('/').last}");
          await storageRef.putFile(image);
          String imageUrl = await storageRef.getDownloadURL();
          imageUrls.add(imageUrl);
        }

        await FirebaseFirestore.instance.collection("parrainageOrphelin").add({
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          title: const Text("Ajouter un message pour parrainnées orphelins"),
          content: Column(
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                try {
                  await FirebaseFirestore.instance.collection("parrainageOrphelin").doc(doc.id).update({
                    "title": titleController.text,
                    "message": messageController.text,
                  });
  ...
  Future<void> _deleteDonation(DocumentSnapshot doc) async {
    try {
      ...
      await FirebaseFirestore.instance.collection("parrainageOrphelin").doc(doc.id).delete();
      if (!mounted) return;
